...
class Train:
    def __init__(self, data_train, data_test, dataset_name, hidden_size=64, BATCH_SIZE=30,
                 GAMMA=0.7, EPS=0.1,
                 ReplayMemorySize=50,
                 TARGET_UPDATE=5,
                 n_actions=3,
        ...
        self.policy_net = Decoder(self.hidden_size)
        self.target_net = Decoder(self.hidden_size)
    ...
    def save_model(self, model):
        ...
        while os.path.exists(
                f'{PATH}{self.DATASET_NAME}; DATA_KIND({self.data_train.data_kind}); CNN-GRU; WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; '
                f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; '
                f'EXPERIMENT({experiment_num}).pkl'):
            experiment_num += 1

        file_name = f'{PATH}{self.DATASET_NAME}; DATA_KIND({self.data_train.data_kind}); CNN-GRU; WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; ' \
                    f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; ' \
                    f'EXPERIMENT({experiment_num}).pkl '
    ...
    def test(self, file_name, action_name, test_type='train'):
        ...
        if file_name is None:
            ...
            file_path = os.path.join(Path(os.path.abspath(os.path.dirname(__file__))).parent.parent,
                                     f'Objects\\CNN-GRU\\{file_name}')

        data = self.data_train if test_type == 'train' else self.data_test

        test_encoder = Encoder(self.window_size, self.hidden_size, device).to(device)
...
# file_name = 'BTC-USD; DATA_KIND(LSTMSequential); CNN-GRU; PredictionStep(None); WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT(1).pkl'
