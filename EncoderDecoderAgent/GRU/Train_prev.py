...
class Train:
    def __init__(self, data_train, data_test, dataset_name, hidden_size=50, transaction_cost=0.0, BATCH_SIZE=30,
                 GAMMA=0.7, EPS=0.1,
                 ReplayMemorySize=50,
                 TARGET_UPDATE=5,
                 n_actions=3,
        ...
        self.policy_net = Decoder(self.hidden_size, self.n_actions).to(device)
        self.target_net = Decoder(self.hidden_size, self.n_actions).to(device)

        # for _, param in self.encoder.named_parameters():
        #     init.normal(param, 0, 1)

        self.seq2seq_policy = Seq2Seq(self.encoder, self.attention, self.policy_net).to(device)
        self.seq2seq_target = Seq2Seq(self.encoder, self.attention, self.target_net).to(device)
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                action, _ = self.seq2seq_policy(state)
    ...
    def optimize_model(self):
        ...
        state_action_values, hidden = self.seq2seq_policy(state_batch)
        ...
        next_state_values_temp, _ = self.seq2seq_target(non_final_next_states)
    ...
    def train(self, num_episodes=50, tensorboard=None):
        for i_episode in tqdm(range(num_episodes)):
            ...
            if tensorboard is not None:
                ...
                for name, param in self.encoder.named_parameters():
                    tensorboard.add_histogram(name, param.grad.data.sum(), i_episode)
    ...
    def save_model(self, model):
        ...
        while os.path.exists(
                f'{PATH}{self.DATASET_NAME}; DATA_KIND({self.data_train.data_kind}); GRU; TransactionCost({self.transaction_cost}); WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; '
                f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; '
                f'EXPERIMENT({experiment_num}).pkl'):
            experiment_num += 1

        file_name = f'{PATH}{self.DATASET_NAME}; DATA_KIND({self.data_train.data_kind}); GRU; TransactionCost({self.transaction_cost}); WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; ' \
                    f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; ' \
                    f'EXPERIMENT({experiment_num}).pkl '
    ...
    def test(self, file_name, action_name, initial_investment=1000, test_type='train'):
        ...
        test_net = Seq2Seq(test_encoder, test_attention, test_decoder)
...
deepRLAgent = Train(dataTrain, dataTest, DATASET_NAME, hidden_size, transaction_cost,
...
# tb = SummaryWriter()
# deepRLAgent.train(100, tb)
# tb.close()

deepRLAgent.train(10)
file_name = None

# file_name = 'AAL; DATA_KIND(LSTMSequential); GRU; TransactionCost(0); WindowSize(10); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT(1).pkl'

# file_name = 'S&P; DATA_KIND(LSTMSequential); GRU; PredictionStep(None); WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'
...
# file_name = 'AAPL; DATA_KIND(LSTMSequential); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'
# file_name = 'AAPL; DATA_KIND(SequencePrediction); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'

# file_name = 'GOOGL; DATA_KIND(LSTMSequential); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'
# file_name = 'GOOGL; DATA_KIND(SequencePrediction); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'

# file_name = 'AAL; DATA_KIND(LSTMSequential); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'
# file_name = 'AAL; DATA_KIND(SequencePrediction); GRU; WindowSize(20); TRAIN_TEST_SPLIT(True); BATCH_SIZE10; GAMMA0.7; EPSILON0.1; REPLAY_MEMORY_SIZE20; C5; N_SARSA10; EXPERIMENT.pkl'
