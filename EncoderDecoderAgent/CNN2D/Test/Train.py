...
from EncoderDecoderAgent.CNN.Test.DQN import DQN
...
class Train:
    def __init__(self, data_train, data_test, dataset_name, BATCH_SIZE=30, GAMMA=0.7, EPS=0.1,
        ...
        self.policy_net = DQN(self.data_train.state_size, self.window_size).to(device)
        self.target_net = DQN(self.data_train.state_size, self.window_size).to(device)
    ...
    def save_model(self, model):
        ...
        while os.path.exists(
                f'{PATH}{self.DATASET_NAME}; CNN-test; WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; '
                f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; '
                f'EXPERIMENT({experiment_num}).pkl'):
            experiment_num += 1

        file_name = f'{PATH}{self.DATASET_NAME}; CNN-test; WindowSize({self.window_size}); TRAIN_TEST_SPLIT({self.train_test_split}); BATCH_SIZE{self.BATCH_SIZE}; GAMMA{self.GAMMA}; EPSILON{self.EPS}; ' \
                    f'REPLAY_MEMORY_SIZE{self.ReplayMemorySize}; C{self.TARGET_UPDATE}; N_SARSA{self.n_step}; ' \
                    f'EXPERIMENT({experiment_num}).pkl '
    ...
    def test(self, file_name, action_name, test_type='train'):
        ...
        test_net = DQN(self.data_train.state_size, self.window_size)
